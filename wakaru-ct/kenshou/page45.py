...
第 VI 部は、<strong>手続きそのものを検査する部</strong>です。最初は <strong>国際単位系 2019</strong> ── 2019 年に SI は、\(c\)、\(\hbar\)、\(e\)、\(k_B\) などの<em>定数の値を固定して</em>単位を定義し直しました。キログラムの原器は廃止され、質量は \(\hbar\) から作られます。つまり ── <strong>「次元付きは帳簿」ということが、国際的に、公式に、宣言された</strong>のです。第3回でこのシリーズが手で引いた線を、<em>世界の度量衡が同じ場所に引いていました</em>。それを数えます。
</div>'''
