...
第 III 部はここまで。第 IV 部では<strong>第3回でやった手術 ── 名前の中に隠れた比較相手を名指しする ── を、他の理論に当てます</strong>。最初は<em>インフレーション</em>。<strong>「地平線問題を解く」という言い方の中に、二つの別物が同じ名前で入っています</strong> ── <em>因果的に繋げること</em>と、<em>ゆらぎのスペクトルを出すこと</em>。第17回で数えた 20 KB を使うと、前者が要求するのは驚くほど少ないことが分かります。<strong>そして \(N=60\) e-folds という数の出どころを、ビットで数え直します。</strong>
</div>'''
