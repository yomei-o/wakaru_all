...
| **考える波**〔※新シリーズ・**完結**〕| 音声を微分すると高音、積分すると低音 ── ならば物理定数はすべて何かの積分値ではないか。その一行から最後まで歩く**全8話・4部構成（完結）**。**第 I 部（道具をつくる）**：48kHz の合成音声を実際に微分・積分して測ると **±6.02 dB/oct・位相 ±90°**（ズレは中心差分の sinc で小数点以下6桁まで説明）→ 階数 α を連続にすると dB/oct=6α、そして **α=−1/2（半積分）は実在する**（ワールブルグ・インピーダンス、電池の研究室で毎日測られている）。**第 II 部（定数を波として見る）**：積分定数の正体は **ω=0 の成分**（1/ω が発散する）→ **物理定数 ＝ 周期が宇宙年齢より長い成分**（2.3×10⁻¹⁸ Hz が限界）→ c は1983年から定義値なので微分は取り決め、意味があるのは無次元量だけ → 各実験を周波数帯に翻訳すると**周期32 Gyr で光時計とオクロ天然原子炉の主役が交代**（卓上の時計が宇宙史とほぼ互角）→ **★ 軸を ln E に変えると当たる**：一ループで **1/α = 137.036 → 128.940**（文献値128.95）、これは ∫β(α)d ln μ そのもの。**第 III 部（物理を波として見る）**：**★ 質量は遮断周波数だった** ── クライン＝ゴルドンの分散関係は導波管と一字一句同じ形で、遮断以下のエバネッセント減衰長がそのまま湯川ポテンシャル（パイ中間子 **1.41 fm**）、E=mc² は ħω_c、E²=(pc)²+(mc²)² は分散関係そのもの → 一方「質量＝重力の積分」は GM/(Rc²)=**0.500000** とぴったり出るが**代入すると G も c も H も消える恒等式**で、検証可能な形（ブランス＝ディッケ）は**カッシーニが 10⁻⁵ で棄却**、「光速＝情報の積分」も c が定義値なので成立しない。**第 IV 部**：成立6・否定3、**境目は「次元」**（当たるのは無次元量、外れるのは次元を持つ量）＋枠組み自体の限界3つ（定常性・線形性・ω=0 に手が届かない）＋**開いたままの扉5つ**（1/f 雑音の起源、分数階と粗視化、α の値の由来、10⁴〜10¹⁰年の周期帯という観測の穴）。**否定された仮説もそのまま掲載**。数値はすべて検証スクリプト6本で計算・実行済み| [開く](https://yomei-o.github.io/wakaru_all/kangaeru-nami/index.html) |
