...
| **わかる c·t=一定**〔※「わかる共形変換」続編・進行中〕| c·t=一定 は、それ自体では何も言わない ── どんな宇宙でも実現できてしまう。だからこそ**あらゆる式に代入できる**。宇宙論をいちばん短く書くための記法として扱い、その「短さ」を情報理論の言葉で測る全50話・6部構成。**第 I 部（記法をつくる）**：メモリ÷演算＝状態方程式（Ω/N=ln2/3π²(1+w)）→ 対数クロック 140 と繰り込み群 73 を割ると膨張則（p=0.513）→ 読者の質問から「c·t=一定 は T=exp(η/η₀) を取ればどんな宇宙でも厳密に成り立つ」（残る主張はただ一つ「その時計が宇宙の年齢である」）→ 消せるものを全部消すと**質量ひとつ**しか残らない → 短さの値段は AIC 1.443 bit / BIC 5.37 bit、当てはまりの損は 154 bit → メモリの 10⁻¹⁸ しか使っていない（＝全BHの地平面を貼ると半径17光年）。**第 II 部（方程式に片っ端から入れる）**：重力（Gは月レーザー測距の上限の1450倍速く変わるのに誰も気づけない。ディラックの大数は**本当にGを変えても両方とも不変**）→ 量子力学（波束は ln t でしか広がらず、物質の共動到達距離は有限、光だけ無限）→ 原子（断熱パラメータ 1.1×10⁻³⁴。アインシュタイン1918の刃が届かない理由）→ 熱と情報（書ける10¹²²、消せる10⁹²。「ランダウアー限界ぴったり」は恒等式だった）→ 光（光子ガスは完全に静止）→ 真空（**順位が逆転し、宇宙定数がいちばん速く育つ**）→ 流体（Re の四部品はバラバラに動くのに和は0）→ 相転移（**ウェイト表そのものに誤差棒** γ_σ=0.0181489）→ 化学と生物（生命は自分がどちらの絵にいるか知り得ない）→ 総括。**触れるのは「大きさ」だけ、「かたち」には手が届かない**（第16回まで公開）| [開く](https://yomei-o.github.io/wakaru_all/wakaru-ct/index.html) |
